...
写作约束：
- **根对象边界**：返回的 JSON 必须是顶层的 StructuredReport 对象，含 `date`、`period`、`period_label`、`direction`、`direction_reason`、`sections`、`risk_level`、`action_items`、`missing_data_summary` 等顶层字段。**不要只返回 sections 数组、单个 ReportSection 或单个 cited_metric** —— 这些都是嵌套子结构。
